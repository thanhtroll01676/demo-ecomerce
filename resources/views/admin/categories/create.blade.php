        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="panel panel-default">
                    <div class="panel-heading text-center">Thêm chuyên mục</div>
                        ...
                        <form action="{{ route('admin.category.store') }}" method="post">
                            ...
                            <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                                ...
                                <input type="text" class="form-control" id="name" name="name"
                                       placeholder="ghi tên chuyên mục vào đây" value="{{ old('name') }}">
                                <span class="help-block">{{ $errors->first('name') }}</span>
                            </div>
                            <div class="form-group {{ $errors->has('order') ? 'has-error' : '' }}">
                                <label for="order">Thứ tự ưu tiên</label>
                                <input type="text" class="form-control" id="order" name="order"
                                       placeholder="ghi thứ tự ưu tiên vào đây" value="{{ old('order') }}">
                                ...
                                <select name="parent" id="parent" class="form-control">
                                    ...
                                    @if(count($categories) > 0)
                                        @foreach($categories as $v)
                                            <option value="{{ $v->id }}" {{ old('parent') == $v->id ? 'selected' : '' }}>{{ $v->name }}</option>
                                        @endforeach
                                    @endif
                                </select>
